...
def get_tcp_three_way_handshake(sip, dip, srcport=4096, dstport=80):
    syn = Ether() / ScapyIP(src=sip,dst=dip) / TCP(sport=srcport, dport=dstport, flags="S")
    syn_ack = Ether() / ScapyIP(src=dip,dst=sip) / TCP(sport=dstport, dport=srcport, ack=syn[TCP].seq + 1, flags="S""A")
    ack = Ether() / ScapyIP(src=sip,dst=dip) / TCP(sport=srcport, seq=syn_ack[TCP].ack, ack=syn_ack[TCP].ack, dport=dstport, flags="A")
