...
int foreach_pcapng_block(uint32_t block_counter, uint32_t block_type, uint32_t block_total_length, unsigned char *data)
{
...
int cpcapng_file_read(char *filename, foreach_pcapng_block_cb pcapng_block_cb)
{
	...
	while (read_length > 0) {
		...
		pcapng_block_cb(block_counter, block_info[0], block_info[1], (unsigned char *)data);
...
{
	return cpcapng_file_read(filename, foreach_pcapng_block);
}
